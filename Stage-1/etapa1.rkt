...
(define (get-men mpref)
  (if (null? mpref)
      null
      (cons (first (first mpref)) (get-men (rest mpref))))) 
...
(define (get-women-helper wpref acc)
  (if (null? wpref)
      (reverse acc)
      (get-women-helper (rest wpref) (cons (first (first wpref)) acc))))

(define (get-women wpref) (get-women-helper wpref null))
...
(define (get-pref-list pref person)
  (if (equal? (first (first pref)) person)
      (rest (first pref))
      (get-pref-list (rest pref) person)))
...
(define (preferable? pref-list x y)
  (or (null? pref-list)
      ; verificam daca y este primul element din lista de preferinte
      ; si daca x apare dupa y. Daca este asa testul va da #f, altfel
      ; daca nu exista un caz in care y apare in fata lui x, va da #t
      (and (not (and (equal? y (first pref-list)) (list?(member x (rest pref-list))))) (preferable? (rest pref-list) x y))))
...
(define (get-partner engagements person)
  (cond
    ((null? engagements) #f)
    ((equal? (caar engagements) person) (cdr (first engagements)))
    (else (get-partner (rest engagements) person))))
